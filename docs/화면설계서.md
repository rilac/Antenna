...
작성 2026-08-28 · 갱신 2026-09-07(모의투자 G 영역) · 기준 [엔테나 API 명세서](앤테나_API_명세서.html) · [ERD v0.9](앤테나_ERD.html) · SSAFY 15기 S15P21A507
...
| **G-02a** | 설계서 §3에 없던 화면을 추가한 것이다 — G-02에서 연습을 고른 사람이 머무는 허브. 연습 주제는 `seasons.title`·`note`·`theme`에서 오며 **화면에 하드코딩하지 않는다**. 대회·시연 시즌은 이 목록에 섞지 않는다(`?mode=PRACTICE`).<br>**2026-09-07 — 연습 시즌 셋이 실제로 열렸다.** 전기·전자 / 화학 / 운송장비·부품, 각 **5종목 · 60게임일 · 초기 예수금 3,000만원**. 기동 시 시더가 만든다 — 관리자 권한 경로가 없어 `POST /admin/seasons` 는 아직 없다. 60게임일은 대회 기본값(120)의 절반인데, 연습은 참가자가 직접 하루씩 넘기므로 120번을 누르게 하면 끝까지 가는 사람이 없다 |
| **G-03** | PRACTICE·DEMO 즉시 `201` / COMPETITION은 **참가비 소각 서명** → `202` → M-02. 참가비는 **비용(소각)**이지 보상이 아니다.<br>**재도전은 모드마다 다르다** — 연습·시연은 회차(`attempt_no`)를 늘려 **여러 번 다시 할 수 있고**, 대회는 `attempt_no=1` 고정으로 시즌당 한 번뿐이다. 유니크는 `(season_id, user_id, attempt_no)` 세 컬럼이라 스키마가 회차를 허용하고, 대회의 1회 제한은 앱이 지킨다. 결과·거래·포지션은 모두 시즌이 아니라 **회차(`participantId`)에 귀속**되므로 응답에도 어느 회차인지 실어야 한다.<br>**시기를 그리지 않는다** — 성격(`title`)·섹터·기간·예수금·종목 수까지만 보여주고 실제 날짜와 연도는 어떤 형태로도 표시하지 않는다.<br>**2026-09-07 — 연습은 종목이 실명이다.** 그래서 연습에서 시기 은닉은 형식이다: 실명과 실제 주가가 함께 나가면 검색 한 번에 날짜가 나온다. 화면이 날짜를 안 그리는 이유는 은닉이 아니라 축이 `gameDay`이기 때문이다. **대회는 진짜로 가린다** — 가명(`A사`) + 섹터 상위 분류. 진행 중인 회차가 있으면 참가 버튼이 **이어서 하기**로 바뀐다.<br>**연습 시즌의 `status` 는 항상 `RUNNING`** 이다 — 열릴 시각이 없어 `SCHEDULED` 를 쓰지 않는다(ERD v0.7). 화면이 "곧 시작" 을 그릴 일이 없다. **`entryFee` 는 아직 어떤 응답에도 오지 않는다**(대회 금액표 D4 미확정) — 대회 카드의 참가비 자리는 값이 올 때까지 비워 둔다 |
| **G-04** | 주문 본문은 `tickerId`·`side`·`qty` 셋 — **게임일 종가 단일가 체결**, 유형·수수료·예약·손절 없음. `/tickers`는 `displayName`만 온다 — **연습은 실명("삼성전자"), 대회는 가명("A사")**이고 원본 종목코드는 어느 쪽도 응답에 없다. 화면은 이 값을 그대로 그리면 되고 모드로 갈라지지 않는다. 진행일 초과분 응답 금지. 보조지표는 프론트 계산. 진행은 `expectedDay` 낙관적 잠금(불일치 409), COMPETITION은 수동 진행 불가.<br>캔들은 `season_prices`의 `open`·`high`·`low`·`close`·`volume`으로 그린다(ERD v0.6에서 추가). 원천에 없는 값은 `null`로 오므로 거래량 막대는 빠질 수 있다.<br>**2026-09-07 — `gameDay`가 1부터가 아니다.** 0 이하가 시즌 시작 전 **워밍업 구간**이고 연습은 120봉이 붙어 온다(ERD v0.8). 워밍업이 없으면 첫날 캔들이 한 개라 이동평균도 MACD도 값이 없다 — 60게임일만 쓰면 MA60은 마지막 하루에만 찍힌다. 차트는 `gameDay > 0`부터를 플레이 구간으로 표시하고 워밍업은 배경으로 둔다.<br>**지표는 프론트가 만든다.** MA 5·20·60 · 볼린저 20/2 · RSI 14 · MACD 12/26/9를 `sim/indicators.ts`가 계산한다. 입력과 같은 길이의 배열을 돌려주고 값이 없는 앞쪽은 `null`이라 캔들 배열과 인덱스가 1:1로 맞는다 |
...
| `CandleChart` | G-04 · G-05 | 모의투자 전용 OHLCV. **절단 지점(진행일) prop 필수** · x축은 날짜가 아니라 `gameDay`이고 0 이하는 워밍업 |
| `TickerLabel` | G-04 · G-05 · G-06 | `displayName`만 받는다. **실명 필드를 받지 않는 타입으로 강제하던 규칙은 폐기**(2026-09-07) — 연습은 `displayName` 자체가 실명이라 뜻이 없어졌다. 응답에 없는 종목코드를 못 받는다는 성질은 그대로다 |
