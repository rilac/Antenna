...
| **G-03** | PRACTICE·DEMO 즉시 `201` / COMPETITION은 **참가비 소각 서명** → `202` → M-02. 참가비는 **비용(소각)**이지 보상이 아니다. 시즌당 1회(UQ), 재참가는 이어하기.<br>**시기를 그리지 않는다** — 성격(`title`)·섹터(상위 분류)·기간·예수금·종목 수까지만 보여주고 실제 날짜와 연도는 어떤 형태로도 표시하지 않는다. 이미 참가한 시즌이면 참가 버튼이 **이어서 하기**로 바뀐다 |
