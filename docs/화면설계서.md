...
작성 2026-08-28 · 기준 [엔테나 API 명세서 v0.3](엔테나_API_명세서_v0.3.md) · SSAFY 15기 S15P21A507

**41개 화면 · 10개 모달 · 9개 영역.** REST 93 + STOMP 1 = 94개 인터페이스가 모두 §8 역매핑 표에 배치된다.
...
| **G-02** | 모드 선택 | `/sim/modes` | JWT | 연습 · 대회 · 시연 3카드 + 모드별 차이 | `GET /seasons?mode=` |
| **G-03** | 시즌 상세·참가 | `/sim/seasons/:id` | **서명**(대회) | 기간 · 초기 예수금 · 참가비 · 대회 시간표 · 참가 | `GET /seasons/{id}` · `POST /seasons/{id}/join` |
...
| **G-03** | PRACTICE·DEMO 즉시 `201` / COMPETITION은 **참가비 소각 서명** → `202` → M-02. 참가비는 **비용(소각)**이지 보상이 아니다. 시즌당 1회(UQ), 재참가는 이어하기 |
| **G-04** | 주문 본문은 `tickerId`·`side`·`qty` 셋 — **게임일 종가 단일가 체결**, 유형·수수료·예약·손절 없음. `/tickers`는 `displayName`만(실명·실제 날짜·언론사 금지). 진행일 초과분 응답 금지. 보조지표는 프론트 계산. 진행은 `expectedDay` 낙관적 잠금(불일치 409), COMPETITION은 수동 진행 불가 |
