...
| **E-04** | 내 채널 설정 | `/me/channel` | JWT | 현재 구독료 + 변경 이력 · 구독료 변경 | `GET`·`PUT /me/channel/fee` |
...
| **H-03** | 환경 설정 | `/settings` | JWT | 프로필 수정 · 알림 3종 on/off · 트레이딩 기본 뷰 · 로그아웃 | `GET`·`PUT /users/me/settings` · `PATCH /users/me` · `GET /users/nickname/availability` · `POST /auth/logout` |
...
| **E-04** | 변경은 **차기 주기부터**. 기존 구독은 만료까지 옛 가격. 변경 시 구독자 전원 알림(`FEE_CHANGED`) |
