...
작성 2026-08-28 · 갱신 2026-09-07(모의투자 G 영역) · 2026-09-11(예측 공개 규칙) · 기준 [엔테나 API 명세서](앤테나_API_명세서.html) · [ERD v0.8](앤테나_ERD.html) · SSAFY 15기 S15P21A507

> **변경 로그 (최신이 위)**
> - 2026-09-11 · 김성령(KSR) · `KSR_ANT-PRED-05-07` — 예측 화면을 API 명세 v0.47 에 맞춤. ① 공개 규칙이 "미판정 403" 에서 **필드 단위**로 바뀜(C-03 · §5 · 오류 표 · `SubscriptionGate`) ② B-03 에 예측 호가창·오늘 판정 블록(개인 비공개) + §8 역매핑 2행, 인터페이스 94 → 96 ③ C-01 `horizon` 5/10/20/60 거래일 → **7/14/30/90 캘린더일**(결정 B5 · DB-01), 슬롯 초과는 소각 토큰 전까지 **409**, `note` **필수**. 근거: 프론트 요청 2026-09-10(MR !160) · 지라 S15P21A507-227·-26.

**42개 화면 · 10개 모달 · 9개 영역.** REST 95 + STOMP 1 = 96개 인터페이스가 모두 §8 역매핑 표에 배치된다.
...
| **B-03** | 종목 상세 | `/stocks/:code` | JWT | 요약 헤더 · 종가 차트 · AI 브리핑 · 문서 요약 카드 · 투자 포인트 3열 · 기업 개요 · 재무 · 밸류에이션 · 경쟁사 · 예측 심리 · **예측 호가창(구간별 인원)** · **오늘 판정** | `GET /stocks/{code}` · `/prices` · `/sentiment` · `/predictions/distribution` · `/predictions/settled-today` · `/briefings` · `/documents` · `/points` · `/profile` · `/financials` · `/valuation` · `/peers` |
...
| **C-03** | 예측 상세 | `/predictions/:id` | JWT | 목표·기준가·진행률 · 방향·목표가(판정 전 구독 게이팅) · 근거 본문(구독 게이팅) · 조작 불가 근거 · 검산 이동 | `GET /predictions/{id}` |
...
| **B-03** | 블록별 원천이 다르다 — 헤더 `/stocks/{code}` · 차트 `/prices`(종가 단일) · 브리핑 `/briefings` · 문서 `/documents`(요약+원문 링크만, 본문 미저장) · 포인트 `/points` · 개요 `/profile` · 재무 `/financials` · 밸류 `/valuation` · 경쟁사 `/peers` · 심리 `/sentiment` · 호가창 `/predictions/distribution` · 오늘 판정 `/predictions/settled-today`. 선택한 `pointId`를 C-01로 인계하며 **별도 API가 없다**.<br>**예측 블록은 개인을 보여주지 않는다**(2026-09-10 개편) — 호가창은 판정 대기 목표가를 **전일 종가 대비 5% 구간**의 인원으로만 낸다(±20% 밖은 양 끝 한 칸 · 경계 가격은 서버가 준다, 화면이 다시 계산하지 않는다). 오늘 판정은 판정 후라 작성자까지 공개. 개인 예측은 E-02·C-03 에서만 본다 |
| **C-01** | `direction` UP/DOWN · `targetPrice` 등록 후 불변 · `horizon` **7/14/30/90 캘린더일 고정**(결정 B5 · DB-01) · `note` **필수**·≤5000자·구독자 전용 · `evidencePointIds`는 B-03 인계. **`commitHash`는 서버 salt 결합이라 클라이언트 계산 불가** → `payload`+`noteHash`까지만 미리보기. 기준가는 **다음 영업일 종가**로 배치 B2 확정 → 등록 직후 `BASE`, 빈 값을 0으로 그리지 않는다. 201 슬롯 내 / 슬롯 초과는 **지금 409 `PREDICTION_SLOT_EXCEEDED`**(소각 토큰 미구현 — 붙으면 202→M-02, 잔액 부족 409) / 401 서명 불일치 |
| **C-02** | 응답은 `id`·`stockCode`·`direction`·`targetPrice`·`horizon`·`status`·`dday`·`errorRate`·`settleDate`. 수정·삭제 버튼 없음 |
| **C-03** | **403 없음 — 필드 단위로 가린다**(§5). 판정 전 비구독자는 `locked: true` + `direction`·`targetPrice` null + `evidences` 빈 배열 → 잠금 카드 + 구독 CTA. 판정 후엔 방향·목표가·근거 포인트 공개. **근거 본문은 항상 구독자 전용**(`noteLocked`) — 만기 리빌 후에도 공개되지 않는다(payload에 `noteHash`만 들어감). 커밋 해시·앵커·서명 주소는 항상 공개. 단 검산 이동(`/proof`)은 판정 전 비구독자에게 403 이라 잠금 상태에서는 버튼을 숨긴다 |
...
| **E-02** | 구독은 즉시 토글이 아니다 — M-03 확인 → 서명 → 202 → M-02 → ACTIVE. 최초 구독 서명에 **오퍼레이터 위임 승인**(`operatorAuthorize`) 포함, 모달에서 고지. 결제 시점 `fee` 박제. 백테스트는 **무료·파라미터 고정**(3개월·상수 원금·수수료 0). 예측 목록은 **잠긴 건도 포함**한다 — 판정 전 비구독자에게는 종목만 보이고 `direction`·`targetPrice` 가 null(`locked: true`). 필터는 `status`(`/predictions/me` 와 같은 어휘). 개인 예측은 여기와 C-03 에서만 본다 |
...
| **G-02a** | 설계서 §3에 없던 화면을 추가한 것이다 — G-02에서 연습을 고른 사람이 머무는 허브. 연습 주제는 `seasons.title`·`note`·`theme`에서 오며 **화면에 하드코딩하지 않는다**. 대회·시연 시즌은 이 목록에 섞지 않는다(`?mode=PRACTICE`).<br>**2026-09-07 — 연습 주제 셋이 실제로 열렸다.** 급락과 반등 / 반도체 실적 구간 / 전기차 섹터 변동성, 각 **최대 200종목(첫 게임일 시총 상위) · 38~62게임일 · 초기 예수금 3,000만원 · 워밍업 30봉**. 기동 시 시더가 만든다 — 관리자 권한 경로가 없어 `POST /admin/seasons` 는 아직 없다. 게임일을 60 안팎으로 짧게 두는 이유 — 연습은 참가자가 직접 하루씩 넘기므로 120번을 누르게 하면 끝까지 가는 사람이 없다.<br>**종목은 실명이다(ERD v0.8).** 가명("A사")은 폐기했다 — 종목을 모르면 업종 연관·실적 공부가 성립하지 않는다. 숨기는 것은 실제 날짜뿐이라 카드·상세·진행 화면 어디에도 달력 날짜·연도를 쓰지 않는다. 주제 업종 밖 종목까지 전부 담으므로 진행 화면(G-04)은 **종목코드·종목명 검색과 업종 필터**를 갖춰야 한다. |
| **G-03** | PRACTICE·DEMO 즉시 `201` / COMPETITION은 **참가비 소각 서명** → `202` → M-02. 참가비는 **비용(소각)**이지 보상이 아니다.<br>**재도전은 모드마다 다르다** — 연습·시연은 회차(`attempt_no`)를 늘려 **여러 번 다시 할 수 있고**, 대회는 `attempt_no=1` 고정으로 시즌당 한 번뿐이다. 유니크는 `(season_id, user_id, attempt_no)` 세 컬럼이라 스키마가 회차를 허용하고, 대회의 1회 제한은 앱이 지킨다. 결과·거래·포지션은 모두 시즌이 아니라 **회차(`participantId`)에 귀속**되므로 응답에도 어느 회차인지 실어야 한다.<br>**시기를 그리지 않는다** — 성격(`title`)·섹터(상위 분류)·기간·예수금·종목 수까지만 보여주고 실제 날짜와 연도는 어떤 형태로도 표시하지 않는다. 진행 중인 회차가 있으면 참가 버튼이 **이어서 하기**로 바뀐다.<br>**연습 시즌의 `status` 는 항상 `RUNNING`** 이다 — 열릴 시각이 없어 `SCHEDULED` 를 쓰지 않는다(ERD v0.7). 화면이 "곧 시작" 을 그릴 일이 없다. **`entryFee` 는 아직 어떤 응답에도 오지 않는다**(대회 금액표 D4 미확정) — 대회 카드의 참가비 자리는 값이 올 때까지 비워 둔다 |
| **G-04** | 주문 본문은 `tickerId`·`side`·`qty` 셋 — **게임일 종가 단일가 체결**, 유형·수수료·예약·손절 없음. `/tickers`는 `code`·`name`·`sector`를 준다 — **종목은 실명**(ERD v0.8), 실제 날짜·언론사만 금지. 200개 안팎이라 **종목코드·종목명 검색 + 업종 필터**가 필수다. `/prices`의 `gameDay ≤ 0`은 워밍업 30봉이다. 진행일 초과분 응답 금지. 보조지표는 프론트 계산. 진행은 `expectedDay` 낙관적 잠금(불일치 409), COMPETITION은 수동 진행 불가.<br>캔들은 `season_prices`의 `open`·`high`·`low`·`close`·`volume`으로 그린다(ERD v0.6에서 추가). 원천에 없는 값은 `null`로 오므로 거래량 막대는 빠질 수 있다 |
...
| **G-08** | 응답 항목만 그린다. **AI 복기는 CLOSED 시 배치 B6 생성** → 생성 전 `null`이면 "생성 중", 온디맨드 버튼 없음. 등급 산식 미확정 |
...
| **I-03** | 시즌 가격은 생성 시 `daily_quotes` 구간을 복사해 **읽기 전용**(seed는 시즌 식별값). `SCHEDULED→RUNNING→CLOSED` 단방향, CLOSED가 결과 확정·복기 생성을 트리거 |
...
| 대상 | 조건 | 응답 | 화면 |
|---|---|:--:|---|
| 미판정 예측(BASE/OPEN)의 방향·목표가·근거 포인트 | 작성자·구독자 외 | `200` + `locked: true`, 필드 null | 잠금 카드 + 구독 CTA. **종목과 존재는 공개** |
| 판정 완료 예측(HIT/MISS) | — | `200` | 방향·목표가 전체 공개 — 목표가가 보이면 기준가와 비교해 방향도 드러나므로 가릴 이유가 없다 |
| 예측 근거 본문 | 작성자·구독자 외 (판정 후에도) | `200` + `noteLocked: true`, `note` null | 잠금 카드. 커밋 해시·앵커는 그대로 노출 |
| 미판정 예측의 검산 데이터(`/proof`) | 작성자·구독자 외 | `403` | payload 에 방향·목표가가 있어 막는다. 잠금 상태에서는 검산 이동 버튼을 숨긴다 |
...
**404가 아니라 잠금이다.** 없는 것처럼 보이면 구독 유인이 사라진다. 예측은 2026-09-11(API v0.47)부터 403 대신 필드 단위로 가린다. 미판정 예측도 익명 집계(`/sentiment`)와 구간별 인원(`/predictions/distribution`)은 공개된다 — 개인이 복원되지 않는 모양일 때만.
...
| `403` | 권한 없음(판정 전 검산 데이터 · 관리자 · 타인 작업) | §5 잠금 카드 |
...
| `SubscriptionGate` | C-03 · E-02 · F-02 | `locked`·`noteLocked`(예측) 또는 `locked`(리포트)를 받아 구독 CTA로 전환 |
...
| `CandleChart` | G-04 · G-05 | 모의투자 전용 OHLCV. **절단 지점(진행일) prop 필수** · x축은 날짜가 아니라 `gameDay`이고 0 이하는 워밍업이라 배경으로 구분한다 · 지표는 `sim/indicators.ts`가 계산한다(MA 5·20·60 · 볼린저 20/2 · RSI 14 · MACD 12/26/9). 입력과 같은 길이의 배열을 돌려주고 값 없는 앞쪽은 `null`이라 캔들 배열과 인덱스가 1:1로 맞는다. 봉이 부족한 지표는 `canShow()`로 감춘다 |
| `TickerLabel` | G-04 · G-05 · G-06 | `code`·`name`·`sector`. 종목은 실명이다(ERD v0.8) — 실제 날짜 필드만 받지 않는 타입으로 강제 |
...
| `GET /stocks/{code}/predictions/distribution` | B-03 |
| `GET /stocks/{code}/predictions/settled-today` | B-03 |
| `GET /anchors` | D-01 |
| `GET /anchors/{id}` | D-02 |
...
| 엔드포인트 | 화면 |
|---|---|
| `GET /seasons` | G-01 · G-02 · G-02a |
| `GET /seasons/{id}` | G-03 |
| `POST /seasons/{id}/join` | G-03 → M-02 |
| `GET /seasons/me` | G-01 · G-02a |
...
| C-04 | AI 유형 분석문 · 섹터별·기간별 적중률 · 근거 사용 패턴 · 확신도 캘리브레이션 · 대표 Forecast · 공유 카드 · 누적 수익률 · 120영업일 | 응답 8필드뿐. horizon은 7·14·30·90(캘린더일) |
