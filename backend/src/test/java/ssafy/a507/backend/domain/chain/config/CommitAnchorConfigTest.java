...
class CommitAnchorConfigTest {
    ...
    @Test
    @DisplayName("ABI에_anchor_rootOf_Anchored가_모두_있다")
    void ABI에_anchor_rootOf_Anchored가_모두_있다() {
        // 컨트랙트 표면이 바뀌면 여기서 먼저 깨져야 한다.
        // 릴레이어가 첫 tx 를 보내는 순간에 알게 되면 늦다.
        assertThat(memberNames("function")).contains("anchor", "rootOf");
        assertThat(memberNames("event")).contains("Anchored");
    }

    @Test
    @DisplayName("anchor는_batchId_merkleRoot_commitCount_3인자다")
    void anchor는_batchId_merkleRoot_commitCount_3인자다() {
        // 배포된 컨트랙트가 API 명세 §4.3 정정본과 같은 시그니처인지 못박는다.
        // batchId 를 서버가 정한다는 결정(plan 대안 B1)이 여기 걸려 있다.
    ...
    @Test
    @DisplayName("Anchored_이벤트의_batchId가_indexed다")
    void Anchored_이벤트의_batchId가_indexed다() {
        // 인덱서(ANT-CHAIN-04)가 batchId 로 로그를 필터링한다. indexed 가 빠지면
        // 전체 로그를 받아 직접 걸러야 하고, 그건 조용히 느려지는 종류의 회귀다.
    ...
    void 커스텀_에러_4종이_ABI에_들어_있다() {
        ...
        // BatchAlreadyAnchored 는 재시도하면 안 되고, 나머지는 배치 로직 버그다.
        assertThat(memberNames("error"))
                .contains("BatchAlreadyAnchored", "InvalidBatchId", "EmptyRoot", "EmptyCommitCount");
    }
