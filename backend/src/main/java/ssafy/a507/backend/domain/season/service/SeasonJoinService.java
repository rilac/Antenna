 ...
 *
 * <p><b>회차.</b> 연습은 여러 번 다시 할 수 있다 — 마지막 회차가 끝났으면(진행일 = 총 게임일)
 * {@code attempt_no + 1} 로 새 회차, 진행 중이면 409. "이어하기" 는 참가가 아니라 그 회차로
 * 들어가는 것이다.
 *
...
public class SeasonJoinService {
    ...
    @Transactional
    public Joined join(Long userId, Long seasonId) {
        ...
        if (last.isPresent() && last.get().getCurrentDay() < season.getLengthDays()) {
