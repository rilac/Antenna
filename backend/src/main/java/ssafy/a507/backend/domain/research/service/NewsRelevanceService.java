...
public class NewsRelevanceService {
    ...
    /**
     * 판정 지시문. 고치면 {@code app.ai.gpu.prompt-version} 을 올린다 — 옛 세대 판정만 다시 돈다.
     *
     * <p>v2(2026-09-11) — v1 에 사회공헌 줄만 더했다. 과거 기사 채점표 두 벌(각 300건, 정답은 사람이
     * GPU 답을 보기 전에 달았다)로 쟀다. 지시문을 고칠 때 보지 않은 새 표본에서 v1 은 정확도 93% · 무관
     * 71건 중 55건을 걸렀고, v2 는 95% · 63건을 걸렀다. 관련 216건 중 버린 것은 4건에서 5건이 됐다.
     *
     * <p>회사 이름 뒤에 업종을 붙이는 안은 두 번 시험하고 버렸다. 이름만 같은 다른 회사(한국동서발전·
     * 효성오앤비)는 더 걸렀지만, 업종이 "기타금융" 인 지주회사(SK·LG·CJ·LS)의 계열사 기사를 절반가량
     * 무관으로 버렸다(관련 48건 중 25건). "계열사는 관련" 이라고 적어도 모델이 괄호 속 업종을 더 믿었다.
     *
     * <p>예시는 v1 에 있던 것만 둔다. 채점표에 든 기사를 예시로 넣으면 재채점 점수가 부풀려진다.
     */
    static final String INSTRUCTION =
            ...
            - 기부·봉사·성금 같은 사회공헌 소식은 "무관" 이다. 투자 판단의 재료가 아니다.
              직원 복지·수상·판촉 행사는 사회공헌이 아니다.
