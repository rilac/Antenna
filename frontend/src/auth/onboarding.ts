/* M-09 온보딩 튜토리얼을 띄울 차례인지 기억한다.
   ...
   그런데 M-09 가 뜨는 자리는 **홈** 이다 — 그 사이에 A-02 닉네임 화면이 끼어
   있어서(RequireAccess 가 닉네임 없는 회원을 거기로 돌려보낸다) 라우터 state 로
   값을 들고 갈 수 없다. 그래서 한 칸 건너 전달할 자리를 둔다.

   왜 라우트가 아닌가
   라우트를 주면 히스토리에 남아 이탈 시 복귀가 어렵다(설계서 §4 M-09).
   그래서 홈 위 오버레이이고, 오버레이는 자기가 뜰 때를 스스로 알 수 없다.

   왜 sessionStorage 인가
   returnTo 와 같은 이유이고 같은 자리다. 로그인 직후의 한 흐름 안에서만 쓰이며
   탭을 닫으면 정리된다. localStorage 에 두면 며칠 뒤 다른 날 홈에 들어왔을 때
