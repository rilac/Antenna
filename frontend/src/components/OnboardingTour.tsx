/* M-09 온보딩 튜토리얼 (오버레이 · 라우트 없음)
   ...
   - **M-01 지갑 연동을 이 안에서 열지 않는다.** 모달 3겹이 되기 때문이다.
     연동을 누르면 onLinkWallet 으로 올려 호출부(홈)가 이걸 닫고 띄운다.
   ...
   돌려보내므로 **홈에 닿은 시점에 닉네임은 이미 있다.** 여기서 폼을 다시 두면
...
export default function OnboardingTour({ onClose, onLinkWallet }: Props) {
  /* user 는 null 일 수 있는 타입이지만, 이 오버레이는 홈에서만 뜨고 홈은
     RequireAccess 뒤에 있어 로그인·닉네임이 이미 확정된 자리다. 그래도
     타입을 우겨 넣지 않고 옵셔널로 읽는다 — 가드가 바뀌면 여기서 터진다. */
  const { user } = useAuth()
